...
class StorageSystem:
    ...
    def get_retrieval_time(self, size):
        ...
        # time in ms !!
        tot_time = 0
        # The paper shows the results for a 10 GB file instead of a 1 GB file which we want
        size_in_gigs = size/(size_in_bytes*10)
        tot_time += size_in_gigs*1400
        ...
        latency = 4.34*size_in_gigs - 0.86
        tot_latency = 0
        if(latency < 0):
            tot_latency = 4.34*size_in_gigs
        else:
